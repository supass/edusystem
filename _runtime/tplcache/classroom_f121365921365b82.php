...
<div class="content">
	<div class="wrap">
    	<ul class="content-nav">
    		...
    		<?php if(is_array($selCate)): ?><?php $i = 0;?><?php $__LIST__ = $selCate?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li>
	        		<a href="javascript:;" onclick="cateBang(this,<?php echo ($vo["zy_video_category_id"]); ?>)"><?php echo ($vo["title"]); ?></a>
	        		<?php if($vo['childlist']){ ?>
			        	<div class="content-nav-child">
			        		<?php if(isset($vo["childlist"])): ?><?php foreach($vo["childlist"] as $key=>$li): ?><a href="javascript:;" id="sona" onclick="cateBang(this,<?php echo ($li["zy_video_category_id"]); ?>)"><?php echo ($li["title"]); ?></a><?php endforeach; ?><?php endif; ?>
		        		</div>
	        		<?php } ?>

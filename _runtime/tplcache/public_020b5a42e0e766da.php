...
<div class="sixin-worap position-rt">
    ...
    <dl>
        ...
        <dd <?php if((MODULE_NAME=="Message" && ACTION_NAME=="index") || ACTION_NAME=="detail"){echo "class='hover'";} ?> ><a href="<?php echo U('public/Message/index');?>">我的私信</a></dd>
        <dd  <?php if(MODULE_NAME=="Message" && ACTION_NAME=="comment"){echo "class='hover'";} ?>>
        <a href="<?php echo U('public/Message/comment');?>">收到的评论</a>
        </dd>
        <dd  <?php if(MODULE_NAME=="Message" && ACTION_NAME=="notify"){echo "class='hover'";} ?>>
        <a href="<?php echo U('public/Message/notify');?>">系统消息</a>
        </dd>
    </dl>
    ...
    <div class="user-con fl">
        <div class="user-con-title"> <a href="#" class="onactive">全部消息：</a></div>
        <!--后台私信-->
        <ul class="user-wenda">
            <?php if(isset($list["data"])): ?><?php foreach($list["data"] as $key=>$l): ?><li class="li">
                <div class="discusslist padding-new"><a target="_blank" title=" " class="other-txt f228" href="#">系统提示</a>
                    <div class="detailmeg">
                        <div class="report">
                            <div class="mynote"><a title="<?php echo getUserName($l.uid);?>" href="<?php echo U('classroom/UserShow/index',array('uid'=>$l['uid']));?>"><?php echo getUserName($l['uid']);?></a>&nbsp;&nbsp;
                                <?php if(!empty($l['uid'])){ ?>
                                管理员：
                                <?php }else{ ?>
                                   系统
                            ...
                            <!--操作删除--><div class="Operation-wd"><a href="javascript:;" onclick="ui.confirm(this,'确定要删除该消息？')" callback="delnotify(<?php echo ($l["id"]); ?>)"><i onclick="ui.confirm(this,'确定要删除该消息？')" callback="delnotify(<?php echo ($l["id"]); ?>)"></i><span onclick="ui.confirm(this,'确定要删除该消息？')" callback="delnotify(<?php echo ($l["id"]); ?>)">删除</span></a></div>
                            <p class="mynote"><a target="_blank" ><?php echo ($l["body"]); ?></a> </p>
                        </div>
                        <div class="fbtime">时间：<span><?php echo (friendlydate($l['ctime'])); ?></span> <?php if($l['is_read'] == 0){ ?><img src="__THEME__/image/icon_new.gif" /><?php } ?></div>
                    </div>
                </div>
